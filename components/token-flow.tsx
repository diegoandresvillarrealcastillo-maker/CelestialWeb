...
export function ResetPassword({ token }: { token?: string }) {
  ...
  return <main className="token-page"><span>✦</span><h1>Nueva contraseña</h1><p>Usa al menos 12 caracteres, mayúscula, número y símbolo.</p>{token && <form action={submit}><label>Contraseña nueva<PasswordField name="password" minLength={12} maxLength={128} required autoComplete="new-password" /></label><button className="button button-primary">Actualizar contraseña</button></form>}{message && <p role="status">{message}</p>}<Link className="text-link" href="/cuenta">Volver a mi cuenta</Link></main>;
}
