...
export interface OrderService {
  ...
  getGuest(orderId: string, guestToken: string): Promise<unknown | null>;
...
export interface AdminService {
  ...
  getOrderNotifications(auth: AuthContext, afterOrderNumber: number): Promise<unknown[]>;
  updateOrder(auth: AuthContext, orderId: string, input: unknown): Promise<unknown>;
  confirmShipping(auth: AuthContext, orderId: string, shippingCop: number): Promise<unknown>;
...
export type Services = { auth: AuthService; products: ProductService; orders: OrderService; admin: AdminService; health: HealthService };
