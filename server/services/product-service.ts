...
export const selectProduct = `
  SELECT p.id, p.external_id AS "externalId", p.slug, p.name, p.description,
         ...
         p.image_path AS image, p.dimensions, p.weight, p.colors, p.fragrances, p.options,
         p.option_prices AS "optionPrices", p.features,
         p.availability, p.collection, p.featured, p.popular, p.reference_image AS "referenceImage",
         p.requires_consultation AS "requiresConsultation", p.active,
