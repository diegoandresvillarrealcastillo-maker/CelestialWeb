...
import { loginLimit, recoveryLimit } from '../middleware/limits.js';
import type { AuthService } from '../services/contracts.js';
import {
  changePasswordSchema, forgotPasswordSchema, loginSchema, profileSchema, resetPasswordSchema,
...
export function authRoutes(service: AuthService, env: AppEnv) {
  ...
  router.get('/me', requireAuth, (request, response) => {
    const { userId, email, roles, fullName, phone, emailVerified } = request.auth!;
    response.json({ user: { userId, email, roles, fullName, phone, emailVerified } });
  });

  router.post('/csrf', requireAuth, async (request, response) => {
  ...
  router.patch('/profile', requireAuth, requireCsrf, async (request, response) => {
    ...
    const { userId, email, roles, fullName, phone, emailVerified } = auth;
    response.json({ user: { userId, email, roles, fullName, phone, emailVerified } });
  });
